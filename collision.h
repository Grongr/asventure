...
void Collision(std::shared_ptr<SpaceShip> ship, std::array<DrawPirateShip, 9> pirates, Quest* quest)
{
    for (int i = 0; i < 9; i++)
    {
        if (std::abs(ship->get_position().x_pos() - pirates[i].GetShip()->get_position().x_pos()) < 20 && std::abs(ship->get_position().y_pos() - pirates[i].GetShip()->get_position().y_pos()) < 20 && !(quest->is_active()))
        {
